...
app.layout = html.Div(
    children=[
        html.Div(
            children=[
                html.H1("Data and Fitted Curve", style={'textAlign': 'center', 'color': '#4A90E2', 'fontSize': '40px'}),
                dcc.Graph(id='graph', style={'height': '60vh', 'marginTop': '20px'}),
            ],
            style={'padding': '30px', 'backgroundColor': '#F4F6F9', 'borderRadius': '10px', 'boxShadow': '0 4px 8px rgba(0, 0, 0, 0.1)'}
        ),
        html.Div(
            children=[
                html.Div(
                    ...
                    style={'padding': '20px', 'backgroundColor': '#fff', 'borderRadius': '8px', 'boxShadow': '0 4px 6px rgba(0, 0, 0, 0.1)', 'marginTop': '20px'}
                ),
                html.Div(id='range-info', style={'fontSize': '16px', 'paddingTop': '10px', 'color': '#777'}),
                html.Div(id='param-div', style={'fontSize': '16px', 'paddingTop': '10px', 'color': '#777'}),
                html.Div(
                    id='current-params',
                    style={'marginTop': '20px', 'fontSize': '18px', 'fontWeight': 'bold', 'color': '#4A90E2'}
                ),  # To display current values of c1, c2, and b
            ],
...
def update_graph(selected_range):
    ...
    fig.add_trace(go.Scatter(x=x_sliced, y=y_sliced, mode='markers', name='Selected Data', marker=dict(color='#007BFF')))

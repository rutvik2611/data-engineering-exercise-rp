...
# Function to fit the model and handle errors
def fit_model(x_data, y_data, y_error):
    # Ensure there's enough data to fit the model
    if len(x_data) < 3:  # Model has 3 parameters
        print("Not enough data points to perform fitting.")
...
def precompute_fitted_curves(x_data, y_data, y_error):
    ...
    for start in range(len(x_data)):
        for end in range(start + 1, len(x_data) + 1):
            ...
            if popt is not None:
                # If fitting was successful, compute fitted curve
            ...
            # Store both raw and fitted data
            fitted_curves[(start, end)] = (x_sliced, y_sliced, y_fit)
